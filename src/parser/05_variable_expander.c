...
/*   Updated: 2025/08/25 20:00:48 by emcorona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	insert_expansion_values(t_token *token)
{
	...
	if (!token || !token->exp_list || token->type == SINGLE_QUOTES)
	{
...
static int	subs_len_asign(t_token *token, int index)
{
	...
	else if (index > 0 && token->raw_tkn[index - 1] == '\\')
